...
async def kospi(ctx, text):
	...
	stock_dict = dict()
	for page in range(1, 5): # 36page까지 하면 11초나 걸림...
		req = requests.get(f"https://finance.naver.com/sise/sise_market_sum.nhn?sosok=1&page={page}")
		html = req.text
		soup = bs(html, "lxml")
	...
	if text.upper() in stock_dict:
		stock_info = stock_dict[text.upper()]
		await ctx.send(f"{ctx.author.mention} KOSDAQ {stock_info[0]}등 {text.upper()} 종목의 현재가는 {stock_info[1]}원이고, 시가총액은 {stock_info[2]}억 원 입니다.")
	else:
		await ctx.send(f"현재 {ctx.author.mention}님이 요청하신 기업에 대한 정보는 없습니다.")
